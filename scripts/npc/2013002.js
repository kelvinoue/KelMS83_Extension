...
function action(mode, type, selection) {
    if (mode < 1)
        ...
        if (cm.getPlayer().getMapId() == 920010100) { //Center tower
            if (status == 0) {
                eim = cm.getEventInstance(); //Added
                if (eim.getIntProperty("statusStg8") != 1) { //Added
                ...
                if (eim.getIntProperty("statusStg8") == 1) {
                    cm.sendYesNo("I have lifted the spell that was blocking the access to the tower's jail storages. You may find some goodies down there... Alternatively, you may choose to leave now. Are you ready to exit?");
                }
            }
            else if (status == 1) {
                    cm.warp(920011300, 0);
            ...
            if (status == 0) 
                ...
                if(cm.getEventInstance().giveEventReward(cm.getPlayer())) {
                    ...
                    cm.sendOk("Please make room in your inventory first.");
                    cm.dispose();
                }
            }
        }
