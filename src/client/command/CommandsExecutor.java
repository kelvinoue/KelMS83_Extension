...
public class CommandsExecutor {
    ...
    private void registerLv0Commands(){
        ...
        //addCommand(new String[]{"language", "lang"}, ChangeLanguageCommand.class); //Renamed from changel
        addCommand("equiplv",  EquipLvCommand.class);
        addCommand("allrates", ShowRatesCommand.class); //Renamed from showrates
        addCommand("rates", RatesCommand.class);
        addCommand("online", OnlineCommand.class);
        //addCommand("report", GmCommand.class); //Renamed from gm
        ...
        addCommand("exp", ToggleExpCommand.class); //Renamed from toggleexp
        //addCommand("mylawn", MapOwnerClaimCommand.class);
        //addCommand("bosshp", BossHpCommand.class);
        //addCommand("mobhp", MobHpCommand.class);
        
        //KelMS custom commands
        addCommand("gm", SelfGmCommand.class); //Allows new chars to make themselves GMs (max lv 4), for testing purposes
    ...
    private void registerLv1Commands() {
        ...
        addCommand("buff", 1, BuffMeCommand.class); //Renamed from buffme
    ...
    private void registerLv2Commands(){
        ...
        addCommand(new String[]{"recharge", "r"}, 2, RechargeCommand.class);
        ...
        addCommand("sbuff", 2, EmpowerMeCommand.class); //Renamed from empowerme
        addCommand("buffall", 2, BuffMapCommand.class); //Renamed from buffmap
        //addCommand("buffme", 2, BuffCommand.class); //Renamed from buff
        //addCommand("bomb", 2, BombCommand.class);
        //addCommand("dc", 2, DcCommand.class);
        addCommand(new String[]{"cleardrops", "cd"}, 2, ClearDropsCommand.class); //Added cd
        addCommand(new String[]{"clearinv", "ci"}, 2, ClearSlotCommand.class); //Renamed from clearslot
        //addCommand("clearsavelocs", 2, ClearSavedLocationsCommand.class);
        addCommand("warp", 2, WarpCommand.class); //Removed map
        addCommand("warphere", 2, SummonCommand.class); //Removed summon
        addCommand("warpto", 2, ReachCommand.class); //Removed reach, follow
        //addCommand("gmshop", 2, GmShopCommand.class);
        addCommand(new String[]{"heal", "h"}, 2, HealCommand.class); //Added h
        addCommand(new String[]{"item", "i"}, 2, ItemCommand.class); //Added i
        //addCommand("drop", 2, ItemDropCommand.class);
        addCommand("level", 2, LevelCommand.class);
        //addCommand("levelpro", 2, LevelProCommand.class);
        addCommand("slots", 2, SetSlotCommand.class); //Renamed from setslot
        addCommand("stats", 2, SetStatCommand.class); //Renamed from setstat
        addCommand("maxstats", 2, MaxStatCommand.class); //Renamed from maxstat
        addCommand(new String[]{"maxskills", "ms"}, 2, MaxSkillCommand.class); //,Renamed from maxskill, added ms
        addCommand(new String[]{"resetskills", "rs"}, 2, ResetSkillCommand.class); //Renamed from resetskill, added rs
    ...
    private void registerLv3Commands() {
        ...
        addCommand("hpmp", 3, MaxHpMpCommand.class); //Renamed from maxhpmp
        ...
        addCommand(new String[]{"togglewhitechat", "wc"}, 3, ChatCommand.class); //Added wc
        addCommand("fame", 3, FameCommand.class);
        addCommand(new String[]{"givenx", "nx"}, 3, GiveNxCommand.class); //Added nx
        //addCommand("givevp", 3, GiveVpCommand.class);
        addCommand(new String[]{"givemeso", "meso"}, 3, GiveMesosCommand.class); //Renamed from givems
        ...
        addCommand(new String[]{"killall", "ka"}, 3, KillAllCommand.class); //Added ka
        ...
        addCommand(new String[]{"healall", "ha"}, 3, HealMapCommand.class); //Renamed from healmap
    ...
    private void registerLv4Commands(){
        ...
        //KelMS custom commands
        addCommand(new String[]{"arm", "a"}, 4, ArmCommand.class); //All-in-one scrolled endgame armor generator based on stat: str/dex/int/luk
        addCommand(new String[]{"char", "c"}, 4, CharCommand.class); //All-in-one job/lv command with scaled hp/mp/ap/sp/weapon + optional godmode
        addCommand(new String[]{"jobids", "j"}, 4, JobIdsCommand.class); //All-in-one job/lv command with scaled hp/mp/ap/sp/weapon + optional godmode
        addCommand(new String[]{"pots", "p"}, 4, PotsCommand.class); //Potions generator: p.elixir, all cure, onyx apples, summoning/magic rocks
        addCommand(new String[]{"pqbypass", "pqb"}, 4, PqBypassCommand.class); //Allows players to skip stages in Kerning/Ludi/Orbis PQs
        addCommand(new String[]{"shop", "s"}, 4, ShopCommand.class); //KelMS shop with clean endgame items, collectibles, and stars/arrows/bullets, etc.
    ...
    private void registerLv6Commands(){
        ...
        addCommand("setgm", 6, SetGmLevelCommand.class); //Renamed from setgmlevel
