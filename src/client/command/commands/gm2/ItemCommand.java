...
public class ItemCommand extends Command {
    ...
    public void execute(MapleClient c, String[] params) {
        ...
        if (params.length < 1) {
            player.yellowMessage("Usage: !item <itemid> <quantity>");
            return;
        }
        ...
        if(ii.getName(itemId) == null) {
            player.message("Item id '" + params[0] + "' does not exist.");
            return;
        }
        ...
        if (YamlConfig.config.server.BLOCK_GENERATE_CASH_ITEM && ii.isCash(itemId)) {
            player.message("You cannot create a cash item with this command.");
            return;
        }
        ...
        if (ItemConstants.isPet(itemId)) {
                if (params.length >= 2){   // thanks to istreety & TacoBell
                        ...
                        player.message("Usage (Pet): !item <itemid> <expiration>");
                        return;        
                }
        }
