...
public class GotoCommand extends Command {
    ...
    {
        setDescription("Teleport to places");
        ...
        try {
            ...
            if (GOTO_TOWNS_INFO == "") { //Added if to prevent double-running the code
                for (Map.Entry<String, Integer> e : towns) {
                    GOTO_TOWNS_INFO += ("'" + e.getKey() + "' - #b" + (MapleMapFactory.loadPlaceName(e.getValue())) + "#k\r\n");
                }
            }
            if (GOTO_AREAS_INFO == "") { //Added if to prevent double-running the code
            ...
            //KelMS custom maps info
            if (GOTO_KELMS_INFO == "") { //Added if to prevent double-running the code
                List<Entry<String, Integer>> jump = new ArrayList<>(GameConstants.GOTO_KELMS.entrySet());
                sortGotoEntries(jump);
                for (Map.Entry<String, Integer> e : jump) {
                    GOTO_KELMS_INFO += ("'" + e.getKey() + "' - #b" + (MapleMapFactory.loadPlaceName(e.getValue())) + "#k\r\n");
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
            ...
            GOTO_KELMS_INFO = "(none)"; //KelMS custom maps info
        }
    }
    
    public static String GOTO_TOWNS_INFO = "";
    public static String GOTO_AREAS_INFO = "";
    public static String GOTO_KELMS_INFO = ""; //KelMS custom maps info
    ...
    public void execute(MapleClient c, String[] params) {
        ...
        if (params.length < 1){
            String sendStr = "Usage: #b!goto <map name>#k. Available areas:\r\n\r\n#rTowns:#k\r\n" + GOTO_TOWNS_INFO;
            if (player.isGM()) {
                sendStr += ("\r\n#rAreas:#k\r\n" + GOTO_AREAS_INFO);
                sendStr += ("\r\n#rOther maps:#k\r\n" + GOTO_KELMS_INFO); //KelMS custom maps info
            }
        ...
        if (player.isGM()) {
            ...
            gotomaps.putAll(GameConstants.GOTO_KELMS); //KelMS custom maps
        } else {
        ...
        if (gotomaps.containsKey(params[0])) {
            ...
            //KelMS custom fixed spawnpoint for jumpquest maps
            if (params[0].contains("jump")) {
